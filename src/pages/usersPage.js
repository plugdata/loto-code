...
function permissionsInfoTemplate() {
  return `
      ...
      <div class="settings-card">
        ...
        <div style="overflow-x: auto;">
          <table class="user-table">
            ...
            <tbody>${usersTableTemplate()}</tbody>
          </table>
        </div>
      </div>
...
async function handleSaveUser(container, modal) {
...
async function handleDeleteUser(id, container) {
...
function initUserEvents(container) {
  ...
  document.getElementById('btn-save-user')?.addEventListener('click', () => {
    handleSaveUser(container, modal);
  });

  container.querySelectorAll('[data-delete-user]').forEach(btn => {
    btn.addEventListener('click', () => {
      handleDeleteUser(btn.dataset.deleteUser, container);
    });
  });
}
